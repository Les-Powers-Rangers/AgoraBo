...
class PdoJeux
{
    ...
    private function __construct()
    {
        ...
        try {
            ...
            PdoJeux::$monPdo = new PDO($_ENV['AGORA_DSN'], $_ENV['AGORA_DB_USER'], $_ENV['AGORA_DB_PWD'], $options);            // configure l'attribut ATTR_ERRMODE pour définir le mode de rapport d'erreurs
     ...
     */
    public function getLesPersonnes_HP()
    {
        ...
        try {
            $resultat = PdoJeux::$monPdo->query($requete);
            $tbPersonnes_HP = $resultat->fetchAll();
            return $tbPersonnes_HP;
        } catch (PDOException $e) {
            die('<div class = "erreur">Erreur dans la requête !<p>'
                . $e->getmessage() . '</p></div>');
        }
    ...
    public function ajouterTournoi($objTournoi)
    {
        try {
            ...
            $requete_prepare->bindParam(':nbParticipants', $objTournoi->NbParticipantsTournoi, Pdo::PARAM_INT);
            $requete_prepare->bindParam(':gain', $objTournoi->GainTournoi, Pdo::PARAM_STR);
...
//            }
    ...
    public function getUnePersonne($loginPersonne, $mdpPersonne)
    {
        try {
            ...
            $requete_prepare->bindParam(':unLoginPersonne', $loginPersonne);
            ...
            if ($utilisateur = $requete_prepare->fetch()) {
                ...
                $mdpQuery = hash('sha512', $mdpPersonne . $utilisateur->selPersonne);
                // le mot de passe enregistré dans la base doit correspondre au hash du (hash
                // transmis concaténé au sel)
                if ($mdpQuery == $utilisateur->mdpPersonne) {
                    return $utilisateur;
                }
            }
            return null;
